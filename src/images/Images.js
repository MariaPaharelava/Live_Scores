export const IMAGES = {
  ONBOARDING_IMAGE: require('./images/onboarding_image.png'),
  HOMESCREEN_IMAGE: require('./images/banner_homeScreen.png'),
  HOMESCREEN2_IMAGE: require('./images/bryant_logo.png'),
  PROFILE_IMAGE: require('./images/photo_profile.png'),
};
export const SPORTS_IMAGES = {
  SOOCER_IMAGE: require('./sports_images/soccer.png'),
  BASKETBALL_IMAGE: require('./sports_images/basketball.png'),
  FOOTBALL_IMAGE: require('./sports_images/football.png'),
  BASEBALL_IMAGE: require('./sports_images/baseball.png'),
  TENNIS_IMAGE: require('./sports_images/tennis.png'),
  VOLLEYBALL_IMAGE: require('./sports_images/volleyball.png'),
  FIELD_IMAGE: require('./images/field.jpg'),
  BASKETBALLFIELD_IMAGE: require('./images/basketballfield.jpeg'),
};
export const TAB_IMAGES = {
  HOME_IMAGE: require('./tab/Home.png'),
  DISCOVERY_IMAGE: require('./tab/Discovery.png'),
  CHART_IMAGE: require('./tab/Chart.png'),
  PROFILE_IMAGE: require('./tab/Profile.png'),
  POST_IMAGE: require('./tab/writing.png'),
};
export const PROFILE_IMAGE = {
  PROFILE_IMAGE: require('./profile/Profile.png'),
  EMAIL_IMAGE: require('./profile/ProfileEmail.png'),
  EDIT_IMAGE: require('./profile/ProfileEdit.png'),
  LOGOUT_IMAGE: require('./profile/ProfileLogout.png'),
  EDITIMAGE_IMAGE: require('./profile/ProfileImageEdit.png'),
  SPORTS_IMAGE: require('./profile/Sports.png'),
};
export const ADMIN_IMAGES = {
  ARROW_IMAGE: require('./admin/Arrow.png'),
  EDIT_IMAGE: require('./admin/Edit.png'),
  TRASHCAN_IMAGE: require('./admin/TrashCan.png'),
  PLUS_IMAGE: require('./admin/Plus.png'),
  HEART_IMAGE: require('./admin/Heart.png'),
  CHART_IMAGE: require('./admin/Chat.png'),
  SEND_IMAGE: require('./admin/send.png'),
  SCROLLDOWN_IMAGE: require('./admin/chevron.png'),
};
